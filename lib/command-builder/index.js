...
class CoapCommandBuilder {
  constructor(coapClientPath, securityId, hubIpAddress, identity, preSharedKey) {
    ...
    this.identity = identity || uuidv1().replace(/-/g, '');
  ...
  createNewDTLSIdentity() {
    return `coap-client -m post -u "Client_identity" -k "${this.securityId}" -e '{"9090":"${this.identity}"}' "coaps://${this.ip}:5684/15011/9063"`;
  }
